...
async function generateTestStatistics() {
  try {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      // Cálculo baseado em um total de usuários conceitual sem definir a variável
      const userBase = 100 + i * 3 + newUsers; // Base conceitual de usuários
      const totalSubscriptions = Math.floor(userBase * (0.1 + Math.random() * 0.1)); // 10-20% de assinantes
      const totalRevenue = totalSubscriptions * (Math.random() * 20 + 30); // R$30-50 por assinatura
      const conversionRate = (totalSubscriptions / userBase) * 100;
